...
#concept-block[
  ...
  Embedding the `JavaScript` code in the `html` file just as follows. That ensures the browser can load the program script and run it. 
  ```js
  ...
  If more codes can't be directly placed in the `<script></script>`, we can import the external JS file.
  ```js
